...
import (
	...
	"github.com/prebid/prebid-server/adapters/zedo"
...
func newAdapterMap(client *http.Client, cfg *config.Configuration, infos adapters.BidderInfos) map[openrtb_ext.BidderName]adaptedBidder {
	return map[openrtb_ext.BidderName]adaptedBidder{
		...
		openrtb_ext.BidderZedo:        adaptBidder(adapters.EnforceBidderInfo(zedo.NewZedoBidder(client, cfg.Adapters[string(openrtb_ext.BidderZedo)].Endpoint), infos[string(openrtb_ext.BidderZedo)]), client),
	}
}
